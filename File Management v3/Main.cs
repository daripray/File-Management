using File_Management_v3.Library;
...
using Status = File_Management_v3.Library.Status;


namespace File_Management_v3
{
    public partial class Main : Form
    {
        private string txtImageExtensionsDefault = "JPG, JPEG, PNG, GIF, BMP, TIFF, TIF, WEBP, HEIC, HEIF, RAW, CR2, NEF, ORF, SR2, ARW, DNG, PSD, ICO, SVG";
        private string txtVideoExtensionsDefault = "MP4, AVI, MKV, MOV, WMV, FLV, M4V, MPG, MPEG, 3GP, WEBM, VOB, RMVB";
        private string txtDocumentExtensionsDefault = "PDF, DOC, DOCX, XLS, XLSX, PPT, PPTX, TXT, RTF, ODT, ODS, ODP, CSV";
        private string txtKeyIncludeDefault = "IMG_, VID_, MOV_, DSC, PXL, JPEG, PHOTO, SAMSUNG, ANDROID, DCIM, CAM, CAMERA, DCM, PIC, MV, IMG_E";
        private string txtKeyExcludeDefault = ".IMG, .VID, .MOV, SCREENSHOT, .TRASHED, TRASHED";

        private string logFilePath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "file_management_log.txt");
        ...
        {
            InitializeComponent();
            currentState = currentAction = Library.Status.Action.None; // Inisialisasi status awal
        ...
        private void InitScan()
        {
            ...
            txtExtImage.Text = txtImageExtensionsDefault;
            txtExtVideo.Text = txtVideoExtensionsDefault;
            txtExtDocument.Text = txtDocumentExtensionsDefault;

            chkKeyIncl.Checked = false;
            chkKeyExcl.Checked = false;
            txtKeyIncl.Enabled = chkKeyIncl.Checked;
            txtKeyExcl.Enabled = chkKeyExcl.Checked;
            txtKeyIncl.Text = txtKeyIncludeDefault;
            txtKeyExcl.Text = txtKeyExcludeDefault;
        ...
        {
            txtCopyPath.Text = "";
            comboBoxCopySubFolder.SelectedIndex = 0;
            checkBoxMoveDeleteFiles.Checked = false;
            checkBoxCopyImages.Checked = false;
            checkBoxCopyVideos.Checked = false;
            checkBoxCopyDocs.Checked = false;
        ...
        #region SCAN FILES
        private bool validateScanParameters()
        {
            bool isValidPath = !string.IsNullOrWhiteSpace(txtScanPath.Text) && System.IO.Directory.Exists(txtScanPath.Text);

            if (!isValidPath)
            {
                MessageBox.Show("Folder Source is not valid!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                btnScanBrowse.Focus();
                return false;
            }
            ...
            if (!chkImage.Checked && !chkVideo.Checked && !chkDocument.Checked)
            {
                MessageBox.Show("Choose at least one file type!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                groupFileType.Focus();
                return false;
            }
            return true;
        ...
        private void btnScan_Click(object sender, EventArgs e)
        {
            ...
            currentAction = Library.Status.Action.Scan;
            currentState = Library.Status.Process.Running;
            WriteLog(" ----------------------------------- ");
        ...
        private void bgWorkerScan_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                ...
                for (int i = 0; i < total; i++)
                {
                    ...
                    {
                        e.Cancel = true;
                        currentState = Library.Status.Process.Canceled;
                        WriteLog(); // akan log: "Proses dihentikan oleh pengguna."
                        currentAction = Library.Status.Action.None; // Reset currentAction ke None setelah dibatalkan
                        return;
                    }
            ...
            catch (Exception ex)
            {
                ...
                e.Cancel = true;
                currentState = Library.Status.Process.Failed;
        ...
        private void bgWorkerScan_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            ...
            if (e.UserState is not FileScanResult result || result == null)
                ...
                else
                {
                    ...
                    currentState = Library.Status.Process.Completed;
                }
            WriteLog(); // akan log sesuai status
            WriteLog(" ----------------------------------- ");
            currentAction = Library.Status.Action.None; // Reset currentAction ke None setelah selesai
        }
        private void btnCancelScan_Click(object sender, EventArgs e)
        {
            if (bgWorkerScan.IsBusy)
            {
                ...
                btnCancelScan.Enabled = false;
                currentState = Library.Status.Process.Stopping; // Set currentState ke Stopping

                WriteLog(); // akan log: "Proses dihentikan oleh pengguna."
                WriteLog(" ----------------------------------- ");
                currentAction = Library.Status.Action.None; // Set currentAction ke Stop
            }
        }
        ...
        private void btnProcess_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    currentAction = radioButtonProcessCopy.Checked ? Library.Status.Action.Copy : Library.Status.Action.Move;
                    currentState = Library.Status.Process.Running;
                    WriteLog(" ----------------------------------- ");
        ...
        private void bgWorkerCopy_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            for (int i = 0; i < total; i++)
            {
                string stateLabel = isMove ? Library.Status.Process.Moving : Library.Status.Process.Copying;
                string stateResultLabel = isMove ? Library.Status.Result.Moved : Library.Status.Result.Copied;
                ...
                bool _currFileStatusOK = _currFileStatus.Equals(Library.Status.Scan.Ok, StringComparison.OrdinalIgnoreCase);
                ...
                {
                    _currRow.Cells[indexer["copyStatus"]].Value += $"{Library.Status.Process.Skipped}";
                    continue;
                }

                // ⛔ Skip file bukan "OK"
                if (_currRow.Cells[indexer["fileStatus"]].Value.ToString() != Library.Status.Scan.Ok)
                {
                    _currRow.Cells[indexer["copyStatus"]].Value += $"{Library.Status.Result.Skipped}";
                    skipCount++;
                    continue;
                }
                ...
                try
                {
                    if (!_currChecked)
                    {
                        ...
                        {
                            _currRow.Cells[indexer["copyStatus"]].Value += $"{Library.Status.Result.Exist} ";
                            if (param.DeleteAfterMove)
                            {
                                File.Delete(_currSourceFilePath); // Hapus file sumber jika deleteAfterMove diaktifkan
                                _currRow.Cells[indexer["copyStatus"]].Value += $"{Library.Status.Result.Deleted} ";
                                if (File.Exists(_currSourceFilePath))
                                {
                                    _currRow.Cells[indexer["copyStatus"]].Value += Library.Status.Process.Failed + " (Failed to delete source file)";
                                    failCount++;
                                    continue;
                                }
                                else
                        ...
                        else if (isFileDuplicateName(_currSourceFilePath, _currDestFilePath))
                        {
                            ...
                            _currRow.Cells[indexer["copyStatus"]].Value += $"{Library.Status.Result.Renamed} ";
                            if (param.DeleteAfterMove)
                            {
                                File.Delete(_currSourceFilePath); // Hapus file sumber jika deleteAfterMove diaktifkan
                                _currRow.Cells[indexer["copyStatus"]].Value += $"{Library.Status.Result.Deleted} ";
                        ...
                        {
                            if (currentAction == Library.Status.Action.Move)
                            {
                                File.Move(_currSourceFilePath, _currDestFilePath);
                                if (File.Exists(_currDestFilePath))
                                {
                                    _currRow.Cells[indexer["copyStatus"]].Value += $"{Library.Status.Result.Moved} ";
                                    if (param.DeleteAfterMove)
                                    {
                                        File.Delete(_currSourceFilePath); // Hapus file sumber jika deleteAfterMove diaktifkan
                                        if (File.Exists(_currSourceFilePath))
                            ...
                            else
                            {
                                ...
                                if (!File.Exists(_currDestFilePath))
                                {
                                    _currRow.Cells[indexer["copyStatus"]].Value += $"{Library.Status.Process.Failed} (Failed to copy file) ";
                                    failCount++;
                                    continue;
                                }
                                _currRow.Cells[indexer["copyStatus"]].Value += $"{Status.Result.Success} ";
                ...
                {
                    _currRow.Cells[indexer["copyStatus"]].Value += $"{Library.Status.Process.Error}: {ex.Message}";
                    failCount++;
                }
        ...
        private void bgWorkerCopy_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            ...
            else
            {
                ...
                currentState = Library.Status.Process.Completed; // Set state ke Completed
            }

            TimeSpan duration = processEndTime - processStartTime;
            string stateLabel = currentAction == Status.Action.Move ? Library.Status.Process.Moving : Library.Status.Process.Copying;
        ...
        private void btnCopyStop_Click(object sender, EventArgs e)
        {
            if (bgWorkerScan.IsBusy)
            {
                ...
                btnCopyStop.Enabled = false;
                currentState = Library.Status.Process.Canceled; // Set currentState ke Stopping
        ...
        private void WriteLog(string message = null)
        {
            ...
            if (string.IsNullOrWhiteSpace(message))
            {
                switch (currentState)
                {
                    case Library.Status.Process.Running:
                    case Library.Status.Process.Completed:
                        message = $"Proses {currentAction.ToUpper()} {(currentState == Library.Status.Process.Running ? "dimulai" : "selesai")}.";
                        break;

                    case Library.Status.Process.Scanning:
                        message = "Memindai file...";
                        break;

                    case Library.Status.Process.Copying:
                        message = "Menyalin file...";
                        break;

                    case Library.Status.Process.Moving:
                        message = "Memindahkan file...";
                        break;

                    case Library.Status.Process.Canceled:
                        message = "Proses dihentikan oleh pengguna.";
                        break;

                    case Library.Status.Process.Error:
        ...
        private Image LoadImageSafe(Parameters.Preview preview)
        {
            try
            {
                using (var fs = new FileStream(preview.FilePath, FileMode.Open, FileAccess.Read))
                {
        ...
        private Image GetVideoThumnail(Parameters.Preview preview)
        {
            try
            {
                using (ShellFile shell = ShellFile.FromFilePath(preview.FilePath))
                {
                    // Implementasi untuk mendapatkan thumbnail video
            ...
            {
                Debug.WriteLine("Error loading video thumbnail: " + ex.Message);
                return null;
            }
        }
        private void WriteMetadataToRichTextBox(Parameters.Preview preview)
        {
            try
            {
                Debug.WriteLine("Start reading metadata...");
                var allMetadata = FileProperty.GetAllMetadata(preview.FilePath);
                var metadata = new MetadataHelper(preview.FilePath);
                if (preview.IsImage)
                {
        ...
        private void dgvScan_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                if (e.RowIndex >= 0 && e.RowIndex < dgvScan.Rows.Count)
                {
                    // Cek jika kolom yang diklik adalah kolom "check"
                    if (e.ColumnIndex == indexer["check"])
                    {
                    ...
                    // Cek jika klik terjadi di kolom selain "check" untuk menampilkan detail
                    if (e.ColumnIndex > 1)
                    {
                        ...
                        string filePath = Path.Combine(
                            dgvScan.Rows[e.RowIndex].Cells[indexer["dirPath"]].Value?.ToString() ?? "",
                            dgvScan.Rows[e.RowIndex].Cells[indexer["name"]].Value?.ToString() ?? ""
                        );
                        ...
                        {
                            ImageAnimator.StopAnimate(picBox.Image, null);
                            picBox.Image.Dispose();
                            picBox.Image = null;
                        }

                        rtBoxMetaFiles.Clear();
                        if (currentAction != Library.Status.Action.None) return;
                        ...
                        Parameters.Preview preview = new Parameters.Preview
                        {
                            FilePath = filePath,
                            IsImage = isImage,
                            IsVideo = isVideo
                        ...
                        if (isImage)
                        {
                            //picBox.Image = LoadImageSafe(preview); // Boleh dipisah untuk duplikat
                            //picBox.SizeMode = PictureBoxSizeMode.Zoom;

                            //untuk gif
                            bool isGif = ext.Equals("GIF", StringComparison.OrdinalIgnoreCase);
                            if (isGif)
                            {
                                picBox.Image = Image.FromFile(preview.FilePath);
                            }
                            else
                            {
                                picBox.Image = LoadImageSafe(preview);
                            }
                        }
                        if (isVideo)
                        {
                            picBox.Image = GetVideoThumnail(preview);
                        }
                        picBox.SizeMode = PictureBoxSizeMode.Zoom;

                        WriteMetadataToRichTextBox(preview);
            ...
            {
                MessageBox.Show($"Error when clicking cell: {ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        {
            if (tabControl1.SelectedTab.Text == "Scan")
            {
            }

            if (tabControl1.SelectedTab.Text == "Process")
            { // Tab Process
                activateBtnProcessData();
            ...
            if (tabControl1.SelectedTab.Text == "Log")
            {
                ReadLog();
            }
        }

        private void onIddleUI()
        {
            btnProcess.Enabled = true;

        }
        private void onProcessUI()
        {
        ...
        {
            // if image double-clicked, open in default viewer
            if (picBox.Image != null)
            {
                System.Diagnostics.Process.Start(new System.Diagnostics.ProcessStartInfo
                {
                    FileName = _lastPreviewFilePath,
                    UseShellExecute = true
                });
            }
            // if image doubleclicked with ctrl key, open in custom preview form ImagePreview
            else if (picBox.Image != null && Control.ModifierKeys == Keys.Control)
            {
                ImagePreview previewForm = new ImagePreview
                {
                    BackgroundImage = picBox.Image
                };
                previewForm.BackgroundImageLayout = ImageLayout.Zoom;
                previewForm.Show();
            }
        }
    }
